...
const SongProgressBar = memo(() => {
  ...
  return (
    <div className='flex items-center justify-between w-full'>
      <div className='text-primary mr-2 text-xs'>{position ? msToTime(position) : '0:00'}</div>
      <div style={{ width: '100%' }}>
        <Slider
          isEnabled
          value={value}
      ...
      <div className='text-primary ml-2 text-xs'>{duration ? msToTime(duration) : '0:00'}</div>
    </div>
  );
});
