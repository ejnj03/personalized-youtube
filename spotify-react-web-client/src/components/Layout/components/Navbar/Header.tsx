...
const Header = ({ opacity }: { opacity: number; title?: string }) => {
  ...
  return (
    <div
      className={`flex r-0 w-full flex-row items-center justify-between bg-surface rounded-t-md z-10`}
      // Opacity-driven backdrop that ramps from transparent → surface as the
      // page scrolls. color-mix keeps the alpha behavior while reading the
      // theme's `--surface` token instead of a hardcoded near-black.
      style={{ backgroundColor: `color-mix(in srgb, var(--surface) ${opacity}%, transparent)` }}
    >
