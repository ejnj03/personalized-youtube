...
import { ChatPanel, PersonalizationRoot, type PageConfig } from '@showcase/sdk';
import { ThemedFavicon } from './personalization/ThemedFavicon';
import { LyricsView } from './components/Lyrics/LyricsView';
import { VideoView } from './components/Video/VideoView';
import { fontCatalog } from './personalization/schemas'
import '@showcase/sdk/dist/styles.css';
  ...
  const routes = useMemo(
    () =>
      [
        ...
        { path: '/lyrics', element: <LyricsView />, public: true },
        { path: '/video', element: <VideoView />, public: true },
...
function App() {
  // PersonalizationRoot starts with host.initialConfig. The chat panel owns
  // mode hydration: on mount it lists modes (auto-creates Default if empty),
  // sets the active mode, and calls `replace(...)` with the per-mode config
  // returned from /api/page. This keeps App.tsx out of the loader plumbing.
  return (
    <ConfigProvider theme={{ token: { fontFamily: 'var(--font-family, SpotifyMixUI)' } }}>
      <PersonalizationRoot host={host} fontCatalog={fontCatalog}>
        ...
        <ChatPanel host={host} pageSlug="spotify" apiUrl="http://localhost:8787/api/chat" />
      </PersonalizationRoot>
    </ConfigProvider>
  );
}
