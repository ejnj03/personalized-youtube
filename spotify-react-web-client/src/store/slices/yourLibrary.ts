...
export const getLibraryItems = createSelector(
  ...
  (user, filter, myAlbums, myArtists, myPlaylists) => {
    // Spotify's playlist list includes anything you FOLLOW, not just ones
    // you created. The clone can only play songs from your own playlists
    // (followed-playlist tracks aren't retrievable here), so the rest are
    // misleading clutter. Narrow to playlists where you're the owner.
    const ownPlaylists = user
      ? myPlaylists.filter((p) => p.owner?.id === user.id)
      : myPlaylists;

    if (filter === 'ALBUMS') return myAlbums;
    if (filter === 'ARTISTS') return myArtists;
    if (filter === 'PLAYLISTS') return ownPlaylists;
    ...
    return [
      ownPlaylists.slice(0, 3),
      likedSongs,
      myAlbums.slice(0, 2),
      ownPlaylists.slice(3, 6),
      myArtists.slice(0, 1),
      myAlbums.slice(2, 5),
      myArtists.slice(1, 2),
      ownPlaylists.slice(6, 10),
      myAlbums.slice(5, 9),
      myArtists.slice(2, 6),
      ownPlaylists.slice(10, 15),
      myAlbums.slice(9, 13),
      myArtists.slice(6, 10),
      ownPlaylists.slice(15, 20),
      myAlbums.slice(13, 17),
      myArtists.slice(10, 14),
      ownPlaylists.slice(20, 25),
      myAlbums.slice(17, 21),
      myArtists.slice(14, 18),
      ownPlaylists.slice(25, 30),
      myAlbums.slice(21, 25),
      myArtists.slice(18, 22),
      ownPlaylists.slice(30, 35),
      myAlbums.slice(25, 43),
      // all the rest
      ownPlaylists.slice(35),
      myArtists.slice(22),
      myAlbums.slice(43),
    ]
      .filter((r) => r)
