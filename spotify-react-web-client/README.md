<!--
  ⚠️ FORK NOTICE
  This is a fork of francoborrelli/spotify-react-web-client (MIT, © Franco Martín
  Borrelli): https://github.com/francoborrelli/spotify-react-web-client
  Modifications in this fork: integrated the @showcase/sdk personalization layer
  (chat panel + theming) and added the server/ backend (chat, lyrics, music-video
  lookup). Upstream MIT license retained in ./LICENSE. The original README follows.
-->

> 🍴 **Fork notice.** This app is a fork of **[francoborrelli/spotify-react-web-client](https://github.com/francoborrelli/spotify-react-web-client)** (MIT, © Franco Martín Borrelli). In this monorepo it's used as a reference host for [`@showcase/sdk`](../packages/sdk/) — we added the chat-driven personalization layer and the `server/` backend. The upstream MIT license is kept in [`./LICENSE`](./LICENSE). _The original project's README is preserved below._
